...
| 阶段 | 状态 |
|---|---|
| 阶段 0 · 纸上作业 | 🔸 **改为可选**(见下方修订说明)→ [`stages/stage-00.md`](stages/stage-00.md) |
| **阶段 1 · 类型基座 + 参考模型** | 🔨 **骨架 + 71 个测试就绪,69 红** → [`stages/stage-01.md`](stages/stage-01.md) |
| 阶段 2–10 | ⏸ 待按新范式重写 |

### 2026-08-13 修订:阶段 1 按 ECE 2400 范式重写

对照 Cornell ECE 2400 的五个 PA 之后,原设计有三处被判定为错的:

| 原设计 | 问题 | 改成 |
|---|---|---|
| 阶段 0 用开放式问题当开工闸门 | **大学项目从不要求写代码前回答开放式设计题**。判断力靠「两份实现 + 对比报告」长出来,不靠猜 | 阶段 0 改为可选;判断挪进每阶段的报告 |
| 阶段 1 一次铺开 18 个 `todo!()` | PA2 原话:「如果你把所有函数实现完再开始测试,你会体验到 Sarlacc 巨坑级别的痛苦」 | 拆成子步 1a–1f,每步一个测试闸门 |
| 思考阶梯「不给答案」 | PA 大方给结构性提示,坑直接点明。**难度放在「做」,不放在「猜」** | 行为规格写全,提示大方给 |

新增的四样东西:

1. **`NaiveBook` 黄金参考模型** —— 故意最笨、显然正确的 `Vec` 全表扫描订单簿。
   阶段 1 就建。后面每个快版本都要拿同一串随机订单流和它对答案。
   不变量断言只能抓结构性错误;参考模型能抓出「成交价错 1 个 tick」这类语义错误
2. **`scripts/`** —— 一任务一脚本。`eval.sh` **测试不过就拒绝评测**
   (「评估一个不正确的程序的速度是没有意义的」),并强制 release build
3. **按订单流模式 × 规模两维评测** —— `insert-only` / `high-cross` / `cancel-heavy`,
   分别压挂单查询、撮合主循环、撤单定位三条路径
4. **报告模板** —— 固定四节 + 4 页上限,核心是加速比那一列和「理论与实测是否吻合」
...
3. stage-01.md    ← 直接开始写代码
   (stage-00.md 可选,想先在纸上过一遍系统就做)
```
...
```bash
cd projects/rung
./scripts/test.sh        # 先看它红:71 个测试,69 红
```
